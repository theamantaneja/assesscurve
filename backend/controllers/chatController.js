...
// Handle explanation and diagram request
const handleExplanationRequest = async (req, res) => {
  const { topic } = req.body;

  const completion = await openai.chat.completions.create({
    model: 'gpt-4o-mini',
    messages: [
      { role: 'system', content: 'You are a knowledgeable assistant skilled in creating diagrams.' },
      { role: 'user', content: `Explain the structure and function of ${topic} with a diagram. Suggest key components for a diagrammatic representation.` }
    ]
  });

  const explanation = completion.choices[0]?.message?.content.trim();
  const diagram = createMermaidDiagramFromExplanation(explanation);
...
const createMermaidDiagramFromExplanation = (explanation) => {
  const components = extractComponents(explanation);

  if (components.length === 0) return 'graph TD; A[No components found] --> B[Check input];';

  let mermaidDiagram = 'graph TD;\n';
  components.forEach((comp, i) => {
    mermaidDiagram += ` ${String.fromCharCode(65 + i)}[${comp}] -->`;
    if (i < components.length - 1) {
      mermaidDiagram += ` ${String.fromCharCode(65 + i + 1)}[${components[i + 1]}];\n`;
    }
  });
  return mermaidDiagram.trim() + ';';
};

const extractComponents = (explanation) => {
  const keywords = explanation.match(/\b(\w+)\b/g);
  return [...new Set(keywords)].slice(0, 5);
};

module.exports = { storeUserResponse, handleFurtherRequests, handleExplanationRequest };
